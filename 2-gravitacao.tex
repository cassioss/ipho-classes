...
\begin{equation}
	dx = \frac{dy}{\sqrt{1-y^2}}\rightarrow x- x_0 = cos^{-1}(y) \label{eq45}
\end{equation}
...
\begin{equation}
	\alpha = Lu - \frac{GMm^2}{L}  \rightarrow d\alpha = Ldu \label{eq46}
\end{equation}

No entanto, esta variável ainda não é adimensional. Para isso, é necessário utilizar:

\begin{equation}
	\beta = \frac{L\alpha}{GMm^2} \rightarrow d\beta = \frac{Ld\alpha}{GMm^2} \label{eq47}
\end{equation}

Utilizando a equação (\ref{eq44}) e rearranjando os termos, podemos chegar que:

\begin{equation}
	d\theta = -\frac{d\beta}{\sqrt{\left(1 + \frac{2EL^2}{G^2M^2m^3}\right) - \beta^2}} \label{eq48}
\end{equation}

Utilizando agora uma última variável:

\begin{equation}
	\epsilon = \sqrt{1 + \frac{2EL^2}{G^2M^2m^3}} \label{eq49}
\end{equation}

Chegamos, finalmente que:

\begin{equation}
	d\theta = -\frac{d\beta}{\sqrt{\epsilon^2 - \beta^2}} \label{eq50}
\end{equation} {\\}

Se integrarmos, conforme mostrado na eq. (\ref{eq42}), chegaremos que:

\begin{equation}
	\theta - \theta_0 = cos^{-1}\left(\frac{\beta}{\epsilon}\right) \rightarrow \frac{\beta}{\epsilon} = cos(\theta - \theta_0) \label{eq51}
\end{equation}
...
\begin{equation}
	\frac{\left(\frac{L^2}{GMm^2}\right)}{r} = 1 - \sqrt{1 + \frac{2EL^2}{G^2M^2m^3}}(cos(\theta - \theta_0)) \label{eq52}
\end{equation}{\\}
...
\begin{equation}
	\Delta A = \frac{r(r+\Delta r)}{2}sen(\Delta \theta) + borda\label{eq56}
\end{equation}
...
\begin{equation}
	r^2 + \left(\frac{GMm}{E}\right)r - \frac{L^2}{2mE} = 0 	\label{eq65}
\end{equation}
...
O que se encontra dentro da raiz lembra muito a equação da excentricidade de uma cônica, presente na eq.(\ref{eq52}):

\begin{equation}
	r = - \frac{GMm}{2E}(1 \pm \epsilon)	\label{eq66}
\end{equation}

Se a órbita for elíptica, utilizando a eq.(\ref{eq55}):

\begin{equation}
	r = a(1 \pm \epsilon) = a \pm c	\label{eq67}
\end{equation}

Esta equação faz sentido. Se considerarmos o eixo principal das primeiras figuras, fica fácil de ver que os pontos extremos com relação a um dos polos estarão nele.
