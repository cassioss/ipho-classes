...
Muitos dos conceitos relacionados a órbitas e à Gravitação requerem um conhecimento básico sobre elipses. Serão mostradas aqui duas formas de se analisar uma elipse matematicamente: a primeira, em coordenadas cartesianas, e a segunda, em coordenadas polares. Após isso, serão feitas as principais análises sobre Gravitação, ligando a elas os conceitos de elipses para descrever órbitas planetárias.

\section{Definição}

A elipse, por definição, é o lugar geométrico (LG) dos pontos cuja soma das distâncias a dois pontos fixos é constante. A trajetória descrita por este LG é fechada, numa forma aproximada a uma circunferência. A elipse degenerada (na qual estes dois pontos fixos coincidem) é de fato uma circunferência. É uma das cônicas mais conhecidas.

\section{Nomenclaturas}

Um desenho simples da elipse que podemos analisar é aquele no qual, em coordenadas cartesianas, a linha determinada pelos pontos fixos (chamada de \textbf{eixo principal}) está alinhada com o eixo \textit{x} ou o eixo \textit{y} (fig. 1).

\begin{figure}[!htb]
    \centering
     \includegraphics[scale=0.5]{elipse0.png}
    {\emph{\caption{Desenho de elipse com o eixo x alinhado com o eixo principal.}}}
	\label{fig1}
\end{figure}
...
\[
	\frac{a^2 - c^2}{a}= r\left(1- \left(\frac{c}{a}\right)cos\theta\right)
\]
...
Agora sim podemos enunciar a 1ª lei de Kepler:{\\}{\\}{\\}
...
Procuramos a área $\Delta A$ varrida por um corpo em seu movimento orbital, após uma passagem de tempo $\Delta t$ e uma variação angular $\Delta \theta$. Da figura acima, podemos obter que:
...
Se lembrarmos da equação (\ref{eq36}):
...
Se vocês se lembrarem da \refequ{eq59}:

\[
	\frac{L}{2m} = \frac{\pi ab}{T}
\]

Utilizando a \refeq{eq59}:
...
Eliminando $b$ com o uso da excentricidade e utilizando a \refeq{eq68}:
...
Esta equação ainda não está muito gentil conosco, pois trabalha com período e semi-eixo maior, quando normalmente só é fornecido um deles (normalmente, o semi-eixo maior). Se isolarmos o período na 3ª Lei de Kepler, da \refeq{eq60}:
...
Analisar a velocidade de escape para uma elipse pediria conhecimento da velocidade radial e da posição, o que costuma ser difícil de analisar, e se a análise for feita para a Terra (com excentricidade da ordem de 0.0167), a órbita é normalmente dita circular. {\\}
...
Para encontrá-la, podemos utilizar a \refeq{eq80} trocando $a$ pelo raio da órbita:

\begin{equation}
	v_{escape} = \froot{2Gm}{a(1+\epsilon)}						\label{eq82}
\end{equation}

Comparando-a com a velocidade orbital no apoastro, presente na \refeq{eq71}:
...
\begin{figure}[!htb]
    \centering
     \includegraphics[scale=0.5]{elipse8.png}
    {\emph{\caption{Problema mais comum envolvendo órbita hiperbólica.}}}
	\label{fig9}
\end{figure}

Um pequeno satélite de massa $m$ chega próximo a um planeta de massa $M >> m$ e raio $R$ após ter transitado uma longa distância determinada por uma reta assíntota distante de $b$ com relação ao centro de massa do corpo de massa $M$. O corpo chega a uma distância mínima $a$ do planeta sem que haja colisão, e por conta da atração gravitacional, o satélite acaba desviando de sua trajetória original por um ângulo $\theta$.{\\}

\bt{\begin{enumerate}
	\item Encontre a velocidade escalar do satélite quando ele estiver mais próximo do planeta.
	\item Determine $a$ em função dos valores fornecidos e da constante gravitacional $G$.
	\item Determine $v_0$ de tal forma que o satélite não colida com o planeta.
	\item Encontre o ângulo de desvio $\theta$ em função das variáveis fornecidas.
\end{enumerate}}

Se pretende resolver os itens acima, não olhe a próxima página (spoilers). Estes são exercícios altamente recomendados.{\\}
...
Para o terceiro item, basta utilizar a \refeq{eq88} com $a \geq R$:
...
Resta agora o último item. Para isso, precisaremos de algumas equações (\refer{eq11}, \refer{eq50}, \refer{eq54}):
